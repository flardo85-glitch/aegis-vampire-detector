...
def format_euro_pdf(amount):
    """Visualizzazione nel PDF (Standard EUR)"""
...
def generate_premium_pdf(data_list, score, loss, ter, yrs, cap):
    ...
    # Analisi Perdita (CORRETTA SENZA TRONCAMENTI)
...
with st.container():
    ...
    st.caption("""
    IL PRESENTE SOFTWARE E' UN ALGORITMO DI AUDIT MATEMATICO INDIPENDENTE.
    - **Assenza di Consulenza:** Le analisi prodotte NON costituiscono consulenza personalizzata (ex Art. 1, c. 5-septies D.Lgs. 58/98).
    - **Natura dei Dati:** Analisi basata su dati storici pubblici. AEGIS non garantisce rendimenti futuri.
    - **Responsabilita':** L'utente dichiara di consultare i documenti informativi ufficiali (KID) prima di ogni decisione.
    """)
    ...
    if isins:
        ...
        if email and "@" in email:
            try:
                ...
                st.download_button("📩 Scarica Perizia Tecnica (PDF)", data=pdf_data, file_name="Perizia_AEGIS.pdf", mime="application/pdf")
